...
import com.saikumar.SoundFusion.repository.IPlayListRepo;
...
public class PlayListService {
    ...
    public String addSongsToPlayList(PlayList playList,List<Song> existingSongs,LocalDateTime creationTime) {
        ...
        for (int i = 0; i < newSongs.size(); i++) {
            try {
                Song song = songService.getSongById(newSongs.get(i).getSongId());
                    if (songService.isValidSong(song.getSongId())) {
                        System.out.println("Song : " + newSongs.get(i).getSongId() + " already exists");
                    }else{
                        newSongs.remove(song);
                        System.out.println("Song : "+song.getSongId()+"doesn't exit");
                    }
              }catch (Exception e){
